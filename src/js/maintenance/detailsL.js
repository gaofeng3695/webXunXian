...
var User = Vue.extend({
    props: ["myMsgdetails"],
    template: '<div class="frameList">' +
        '<div class="frameListHalf">' +
        '<div class="frameListLeft">用户姓名</div>' +
...
var childComponent = Vue.extend({
    props: ["myOrderdeta"],
    template: '<div class="frameDetailsTitle">维修费用</div>' +
        ...
        '<div class="frameListRight">' +
        '<p>{{totalCost | pliceNumber}}  （{{captialTotalCost}}）人民币</p>' +
        '</div></div>' +
        '<div class="frameList">' +
        ...
        '<li class="event_pic_list" v-for="(picTeam,index) in pic">' +
        '<img :src="imgSrc(index)" data-original="/cloudlink-core-file/file/downLoad?fileId={{index}}" @click="previewPicture($event)" alt=""/>' +
        '</li>' +
        '</ul>' +
        '</div></div>' +
        ...
        '<div class="frameListRight">' +
        '<ul>' +
        '<li class="event_pic_list" v-for="(picTeam,index) in signature">' +
        '<img :src="imgSrc(index)" data-original="/cloudlink-core-file/file/downLoad?fileId={{index}}" @click="previewPicture($event)" alt=""/>' +
        '</li>' +
        '</ul>' +
        '</div></div>' +
...
var Axis = {
    props: ["myMsgaxis"],
    template: '<div class="axisDay" v-for="teamarr in dateArr" v-bind:dateName="teamarr.id">\
...
var detailsV = new Vue({
    ...
    computed: {
        statusName: function() {
            if (this.dataBasics.status == 1) {
                return "待维修";
            } else if (this.dataBasics.status == 2) {
                return "已完成";
            } else {
                return "---";
            }
        ...
        orderShow: function() {
            if (this.dataBasics.workRecordId == "" || this.dataBasics.workRecordId == null || this.dataBasics.workRecordId == undefined) {
                return false;
            } else {
                return true;
            }
